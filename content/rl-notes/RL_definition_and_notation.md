...
11. RL的方法分为model-free和model-base。意思分别是：
    1. model-free：智能体**不尝试去学习**环境是怎么运作的（不学 $P(s'|s,a)$）。它直接学习一个从状态到佛能够做的映射$\pi(a|s)$或当前状态下动作的价值$Q(s,a)$。
    2. model-base:智能体**先构建一个环境的模型**（可能是已知的，也可能是学出来的），然后在脑子里**推演**未来，基于推演结果来做决策。也即是学一个映射$P(s‘|s,a)$。
    3. 二者的优缺点：
