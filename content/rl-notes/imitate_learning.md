...
假设：$\pi_{\theta}(a\neq \pi^{*}(s)\mid s)\le\epsilon$，对任意$s\sim P_{train}(s)$。注意，这里的$P_{train}$是代表了客观世界的规律，并不只局限于训练集，具体如图：![](/imgs/imgs4rl-note/2.png)
    ...
    $$
    \mid\mid p-q\mid\mid =\sum_x \mid p(x)-q(x)\mid=\int\mid p(x)-q(x)\mid dx
...
7. 行为克隆的常见优化方法之三：Dagger。从训练数据入手收集更充分地数据对抗分布迁移。使用现有的agent policy搜集observation,又用专家给出正确的action，既避免了分布迁移，又保证了标签的正确性，这就是dagger的精髓。算法如下图：![](/imgs/imgs4rl-note/5.png)然后是几个natation的澄清：
...
1. 虽说运行策略会得到一条轨迹，但我们组织$p_{train}$并不需要按轨迹来，只用以（state,action）成对的无序的字典组织就可以了
